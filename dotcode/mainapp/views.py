...
def Add_reply(request,id):
    Message.objects.create(message = request.POST['masg'], sender = User.objects.get(id = request.session['user_id']))
    return redirect('/main/profile/'+str(id))
def Add_reply2(request,id):
    Message.objects.create(message = request.POST['masg'], sender = User.objects.get(id = request.session['user_id']))
    return redirect('/main/client_profile/'+str(id))